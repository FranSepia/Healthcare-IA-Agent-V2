(() => {
  ...
  function roiDashboardHTML(){
    ...
    const potentialValue=all.reduce((sum,o)=>{const match=/\$([0-9.]+)(M|K)?/.exec(o.value||'');if(!match)return sum;const mult=match[2]==='M'?1e6:match[2]==='K'?1e3:1;return sum+Number(match[1])*mult},0);
