...
(() => {
  ...
  function decisionQualityCard() {
    ...
    const pipeline = typeof window.pipelineStats === 'function' ? window.pipelineStats() : null;
    const card = { title: 'Pursuit decision quality', sub: 'How qualified pursuits progress after human review.', note: pipeline && pipeline.hasExamples ? 'Includes example data' : '' };
  ...
  function describeForCopilot() {
    ...
    return {
      ...
      pursuitDecisionQuality: dq ? { chart: 'Card "Pursuit decision quality"', isExampleData: !!(window.pipelineStats && window.pipelineStats().hasExamples), how: 'Conversion between stages of the pipeline (items marked example are illustrative): the share of pursuits that reached a stage and then also reached the next one.', steps: dq.steps, strongestHandoff: dq.best } : null,
