...
(() => {
  ...
  function render() {
    ...
    q('.pw-readiness', root).innerHTML = `<small>PROPOSAL READINESS</small><b>${r.pct}%</b><i><u style="width:${r.pct}%"></u></i><span>${r.label} · ${r.open} open item${r.open === 1 ? '' : 's'}</span><em title="Evidence ${r.parts.evidence}% · Team ${r.parts.team}% · Information ${r.parts.information}% · Time ${r.parts.time}%">Evidence ${r.parts.evidence} · Team ${r.parts.team} · Info ${r.parts.information} · Time ${r.parts.time}</em>`;
