...
(() => {
  ...
  function chartCards() {
    ...
    return {
      ...
      pipelineOverview: pipeline ? { title: 'Pipeline overview', sub: 'Opportunities currently in each stage.', note: pipeline.hasExamples ? 'Includes example data' : '', body: funnelHTML(pipeline.stages, pipeline.counts) } : null,
  ...
  function describeForCopilot() {
    ...
    return {
      ...
      pipelineOverview: d.pipeline ? { chart: 'Funnel "Pipeline overview"', how: 'Opportunities currently in each pipeline stage (Qualified = recommended by the latest search; later stages saved by the team).', isExampleData: !!d.pipeline.hasExamples, stages: d.pipeline.stages, counts: d.pipeline.counts } : null,
