(() => {
  ...
  function buildCopilotContext(){
    ...
    return {
      ...
      pipeline: pipeline ? { note: 'Pipeline: Qualified = opportunities the latest search recommends; Internal review, Proposal and Submitted are saved by the team in Firestore. Items with example:true are illustrative examples shown for preview, not real opportunities.', stageOrder: pipeline.stages, stageCounts: pipeline.counts, totalPipelineValue: pipeline.fmtValue(pipeline.totalValue), proposals: pipeline.items } : null,
