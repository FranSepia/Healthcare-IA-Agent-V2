(() => {
  ...
  function buildCopilotContext(){
    ...
    return {
      ...
      pipeline: pipeline ? { note: 'Live pipeline: Qualified = opportunities the latest search recommends; Internal review, Proposal and Submitted are saved by the team in Firestore.', stageOrder: pipeline.stages, stageCounts: pipeline.counts, totalPipelineValue: pipeline.fmtValue(pipeline.totalValue), proposals: pipeline.items } : null,
