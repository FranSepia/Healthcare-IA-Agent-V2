(() => {
  ...
  function mergeOpportunityNavigation(){
    ...
    if(!q('#opportunityHubTabs')){
      ...
      nav.innerHTML='<button class="active" data-hub="today">Today <b>9</b></button><button data-hub="all">All opportunities <b>9</b></button><button data-hub="discarded">Discarded <b>37</b></button><button data-hub="history">History</button>';
  ...
  function hubTabs(active){
    return `<nav class="opportunity-hub-tabs page-shell embedded">${[['today','Today','9'],['all','All opportunities','9'],['discarded','Discarded','37'],['history','History','']].map(x=>`<button data-hub="${x[0]}" class="${x[0]===active?'active':''}">${x[1]} ${x[2]?`<b>${x[2]}</b>`:''}</button>`).join('')}</nav>`;
  }
  ...
  function installDrawer(){
    window.openDrawer=o=>{
      ...
      const shown=records.slice(0,5),at=shown.findIndex(x=>x.id===o.id);q('#drawerPrevious').onclick=()=>window.openDrawer(shown[(at-1+shown.length)%shown.length]);q('#drawerNext').onclick=()=>window.openDrawer(shown[(at+1)%shown.length]);
    };
  }
